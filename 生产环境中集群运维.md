...
- 服务端：
  ...
  - 降低CPU和存储开销：减少不必要的分词；避免不必须要的doc_values；文档的字段尽量保证相同的顺，可以提高文档的压缩率
  - 调整bulk线程池和队列
  - 做到写入和分片的负载均衡，实现水平扩展
...
- 牺牲可靠性：修改translog设置
  - index.translog.durability：默认是request，每次请求的时候都会罗盘。设置成async，异步写入
...
- 严禁使用*开头的通配符Terms查询
- 优化分片：避免过多的分片（一个查询需要访问所有的分片），控制一个分片的大小（搜索类20GB，日志类40GB）；对于基于时间序列的索引，将只读的索引进行force merge，减少segment数量
...
- 现象：集群整体相应缓慢，也没有特别多的数据读写，但是发现节点在持续金西行Full GC
...
- 当一个数据节点上有过多的Hot Shards，可以手动分配分片到特定的节点
...
- 控制Allocation和Recovery的速率保障集群的正常运行
